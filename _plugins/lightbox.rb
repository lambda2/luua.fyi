...
module Jekyll
  class LightboxTag < Liquid::Tag
    ...
    def render(context)
      ...
      l = (url.split("/").length - 1)
      puts "url => #{url}"
      puts "context.registers[:page] => #{context.registers[:page]}"
      relative = l > 0 ? "../" * l : ''
      src = File.join(relative, @path == nil ? '' : @path)
      thumbSrc = File.join(relative, @thumb == nil ? '' : @thumb)
      %(<a href="#{src}" data-lightbox="#{@data}" title="#{@title}" data-title="#{@title}"><img src="#{thumbSrc}" alt="#{@alt || @title}" class="#{@class}" style="#{@img_style}"/></a>)
    end
  end
end
